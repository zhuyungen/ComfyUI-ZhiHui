    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            api.addEventListener("zh_drawing_board_input", (event) => {
                ...
                img.onload = () => {
                    ...
                    const el = {
                        id: uid("image"), kind: "image", url,
                        _img: img, x: 0, y: 0, w: dw, h: dh,
                        rotation: 0, opacity: 1,
                    };
                    const firstLayer = state.layers[0];
                    if (firstLayer.elements.length === 0) {
                        firstLayer.elements.push(el);
                        state.activeLayerId = firstLayer.id;
                    } else {
                        const layer = newLayer(state.layers.length + 1);
                        layer.name = "输入图片";
        ...
        nodeType.prototype._openBoard = function () {
            ...
            runBtn.onclick = async () => {
                runBtn.disabled = true; runBtn.textContent = "导出中...";
                try {
                    ...
                    statusTxt.textContent = "导出失败: " + e.message;
                    runBtn.textContent = "✅ 输出图像";
            ...
            canvas.onpointerup = e => {
                if (state.tool === "lasso" && drawing && state.lasso?.length >= 3) {
                    ...
                    if (shoelace(state.lasso) < 16) { state.lasso = null; }
                }
            ...
            const onKeyDown = e => {
                ...
                if (e.key === "Escape") { document.body.removeChild(panel); document.removeEventListener("keydown", onKeyDown); return; }
